 ...
 * @author ViiSE
 */
public class ProcessorConfirmSoftCheckProductsTestNG {
    ...
    @BeforeClass
    @Parameters({"ip", "port", "answerConfirmSC"})
    ...
    public void apply(long delayMessageDisplay) {
        testBegin("ProcessorConfirmSoftCheckProducts", "apply()");
        ...
        products.add(ProductDTOInit.productDTO("11111", "10"));
        products.add(ProductDTOInit.productDTO("22222", "20"));
        
        Processor proc = new ProcessorConfirmSoftCheckProductsDefaultImpl(products);
        proc.apply(botDAO, msgSender, msgRecipient, delayMessageDisplay);
        
        testEnd("ProcessorConfirmSoftCheckProducts", "apply()");
    }
}
