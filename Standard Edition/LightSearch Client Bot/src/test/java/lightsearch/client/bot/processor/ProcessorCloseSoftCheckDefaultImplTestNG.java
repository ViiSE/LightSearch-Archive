 ...
 * @author ViiSE
 */
public class ProcessorCloseSoftCheckDefaultImplTestNG {
    ...
    @BeforeClass
    @Parameters({"ip", "port", "answerCloseSC"})
    ...
    @Test
    @Parameters({"delayMessageDisplay", "delivery"})
    public void apply(long delayMessageDisplay, String delivery) {
        testBegin("ProcessorCloseSoftCheckDefaultImpl", "apply()");
        ...
        Processor proc = new ProcessorCloseSoftCheckDefaultImpl(delivery);
        proc.apply(botDAO, msgSender, msgRecipient, delayMessageDisplay);
        
        testEnd("ProcessorCloseSoftCheckDefaultImpl", "apply()");
    }
}
