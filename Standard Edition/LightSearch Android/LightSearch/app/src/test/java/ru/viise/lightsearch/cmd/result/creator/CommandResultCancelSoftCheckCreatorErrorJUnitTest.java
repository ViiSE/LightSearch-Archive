...
public class CommandResultCancelSoftCheckCreatorErrorJUnitTest {

    private CommandResultCreator cmdResCr;

    @Before
    public void setUp() {
        cmdResCr = CommandResultCreatorInit.commandResultCancelSoftCheckCreator(
                false, "Cancel soft check failed.", null, false);
    }

    @Test
    public void createCommandResult() {
        testBegin("CommandResultCancelSoftCheckCreatorError", "createCommandResult()");

        try {
            CommandResult cmdRes = cmdResCr.createCommandResult();
            assertThat(cmdRes).isNotNull();
            assertThat(cmdRes.message()).isNotNull();
            assertThat(cmdRes.message()).isEqualTo("Cancel soft check failed.");
        ...
        testEnd("CommandResultCancelSoftCheckCreatorError", "createCommandResult()");
    }
}
