...
public class MessageOpenSoftCheckJUnitTest {
    ...
    @Before
    public void setUp() {
        IMEI = cmdOpenSCDTORule.getIMEI();
        cmdOpenSCDTO = cmdOpenSCDTORule.getCommandOpenSoftCheckDTO();
    }

    @Test
    public void message() {
        testBegin("MessageOpenSoftCheck", "message()");

        MessageOpenSoftCheck msgOpenSC =
                MessageOpenSoftCheckInit.messageOpenSoftCheck(IMEI, cmdOpenSCDTO);
        assertThat(msgOpenSC).isNotNull();

        String rawMessage = msgOpenSC.message();
        assertThat(rawMessage).isNotNull();
        assertThat(rawMessage).isNotEmpty();

        System.out.println("Message: " + rawMessage);

        testEnd("MessageOpenSoftCheck", "message()");
    }
}
