...
/**
 * Обработчик команды привязки товара.
 * @author ViiSE
 * @since 1.0.0
 */
public class BindProcessor extends AbstractProcessorClient {

    private final ClientDAO clientDAO;
    private final CurrentDateTime currentDateTime;
    private final DatabaseRecordIdentifier databaseRecordIdentifier;

    public BindProcessor(LightSearchServerDTO serverDTO, LightSearchChecker checker,
    ...
    public CommandResult apply(ClientCommand clientCommand) {
        if(!super.checker.isNull(clientCommand.IMEI(), clientCommand.barcode(), clientCommand.factoryBarcode())) {
            if(!serverDTO.blacklist().contains(clientCommand.IMEI())) {
                try {
                    DatabaseCommandMessage dbCmdMessage = DatabaseCommandMessageInit.databaseCommandMessageBind(
                            clientCommand.command(),
                            clientCommand.IMEI(),
                            clientCommand.factoryBarcode(),
                            clientCommand.barcode(),
